...
func TestEventGenerator(t *testing.T) {
	...
	for _, type_ := range []EventType{
		...
		parsedEvent, err := NewHcnetAssetContractEvent(&event, passphrase)
...
func TestSACTransferEvent(t *testing.T) {
	...
	sacEvent, err := NewHcnetAssetContractEvent(&xdrEvent, passphrase)
...
func TestSACEventCreation(t *testing.T) {
	...
	t.Run("wrong passphrase", func(t *testing.T) {
		...
		_, err := NewHcnetAssetContractEvent(&xdrEvent, "different")
		require.Error(t, err)
	})
	...
	t.Run("bad asset binary", func(t *testing.T) {
		...
		_, err := NewHcnetAssetContractEvent(&xdrEvent, passphrase)
		require.Error(t, err)
	})
...
func TestFuzzingSACEventParser(t *testing.T) {
	...
	for i := 0; i < 100_000; i++ {
		...
		NewHcnetAssetContractEvent(&event, "passphrase")
	}
}
