...
func NewHcnetAssetContractEvent(event *Event, networkPassphrase string) (HcnetAssetContractEvent, error) {
	...
	if event.Type != xdr.ContractEventTypeContract || event.ContractId == nil || event.Body.V != 0 {
		return evt, ErrNotHcnetAssetContract
	}
	...
	if eventType, found := HCNET_ASSET_CONTRACT_TOPICS[fn]; !found {
		return evt, ErrNotHcnetAssetContract
	} else {
		evt.Type = eventType
	}
	...
	if err != nil {
		return evt, errors.Wrap(ErrNotHcnetAssetContract, err.Error())
	}
...
func parseCanonicalAsset(assetStr string) (*xdr.Asset, error) {
	...
	//  https://hcnet.org/protocol/sep-11#alphanum4-alphanum12
