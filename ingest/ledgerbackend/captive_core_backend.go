...
func (c *CaptiveHcnetCore) roundDownToFirstReplayAfterCheckpointStart(ledger uint32) uint32 {
...
//
// Except for the Close function, CaptiveHcnetCore is not thread-safe and should
...
type CaptiveHcnetCore struct {
	...
	// cancel is the CancelFunc for context which controls the lifetime of a CaptiveHcnetCore instance.
	// Once it is invoked CaptiveHcnetCore will not be able to stream ledgers from Hcnet Core or
	// spawn new instances of Hcnet Core.
	cancel context.CancelFunc

	hcnetCoreRunner hcnetCoreRunnerInterface
	// hcnetCoreLock protects access to hcnetCoreRunner. When the read lock
	// is acquired hcnetCoreRunner can be accessed. When the write lock is acquired
	// hcnetCoreRunner can be updated.
	...
	// such as writing Prometheus metric captive_hcnet_core_latest_ledger.
...
func NewCaptive(config CaptiveCoreConfig) (*CaptiveHcnetCore, error) {
	...
	if config.Toml != nil && config.Toml.HTTPPort != 0 {
		c.hcnetCoreClient = &hcnetcore.Client{
			HTTP: &http.Client{
...
func (c *CaptiveHcnetCore) coreSyncedMetric() float64 {
	if c.hcnetCoreClient == nil {
		return -2
	}

	info, err := c.hcnetCoreClient.Info(c.config.Context)
	if err != nil {
		return -1
	}
...
func (c *CaptiveHcnetCore) coreVersionMetric() float64 {
	if c.hcnetCoreClient == nil {
		return -2
	}
...
func (c *CaptiveHcnetCore) registerMetrics(registry *prometheus.Registry, namespace string) {
	coreSynced := prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			Namespace: namespace, Subsystem: "ingest", Name: "captive_hcnet_core_synced",
	...
	supportedProtocolVersion := prometheus.NewGaugeFunc(
		prometheus.GaugeOpts{
			Namespace: namespace, Subsystem: "ingest", Name: "captive_hcnet_core_supported_protocol_version",
	...
	latestLedger := prometheus.NewGaugeFunc(prometheus.GaugeOpts{
		Namespace: namespace, Subsystem: "ingest", Name: "captive_hcnet_core_latest_ledger",
...
func (c *CaptiveHcnetCore) openOnlineReplaySubprocess(ctx context.Context, from uint32) error {
...
// This is done because `nextLedger` is 0 between the moment Hcnet-Core is
// started and streaming the first ledger (in such case we return first ledger
// in requested range).
func (c *CaptiveHcnetCore) nextExpectedSequence() uint32 {
...
func (c *CaptiveHcnetCore) startPreparingRange(ctx context.Context, ledgerRange Range) (bool, error) {
	c.hcnetCoreLock.Lock()
	defer c.hcnetCoreLock.Unlock()
	...
	if c.hcnetCoreRunner != nil {
		if err := c.hcnetCoreRunner.close(); err != nil {
			return false, errors.Wrap(err, "error closing existing session")
		}

		// Make sure Hcnet-Core is terminated before starting a new instance.
		processExited, _ := c.hcnetCoreRunner.getProcessExitError()
		if !processExited {
			return false, errors.New("the previous Hcnet-Core instance is still running")
		}
	}
...
//
// Because ledger data is streamed from Hcnet-Core sequentially, users should
...
func (c *CaptiveHcnetCore) GetLedger(ctx context.Context, sequence uint32) (xdr.LedgerCloseMeta, error) {
	...
	if c.hcnetCoreRunner == nil {
		return xdr.LedgerCloseMeta{}, errors.New("hcnet-core cannot be nil, call PrepareRange first")
	}
	if c.closed {
		return xdr.LedgerCloseMeta{}, errors.New("hcnet-core has an error, call PrepareRange first")
	}
	...
	for {
		...
		case result, ok := <-c.hcnetCoreRunner.getMetaPipe():
...
func (c *CaptiveHcnetCore) handleMetaPipeResult(sequence uint32, result metaResult, ok bool) (bool, xdr.LedgerCloseMeta, error) {
	if err := c.checkMetaPipeResult(result, ok); err != nil {
		c.hcnetCoreRunner.close()
...
func (c *CaptiveHcnetCore) checkMetaPipeResult(result metaResult, ok bool) error {
	// There are 4 error scenarios we check for:
	// 1. User initiated shutdown by canceling the parent context or calling Close().
	// 2. The hcnet core process exited unexpectedly with an error message.
	// 3. Some error was encountered while consuming the ledgers emitted by captive core (e.g. parsing invalid xdr)
	// 4. The hcnet core process exited unexpectedly without an error message
	if err := c.hcnetCoreRunner.context().Err(); err != nil {
		// Case 1 - User initiated shutdown by canceling the parent context or calling Close()
		return err
	}
	if !ok || result.err != nil {
		exited, err := c.hcnetCoreRunner.getProcessExitError()
		if exited && err != nil {
			// Case 2 - The hcnet core process exited unexpectedly with an error message
			return errors.Wrap(err, "hcnet core exited unexpectedly")
		} else if result.err != nil {
			// Case 3 - Some error was encountered while consuming the ledger stream emitted by captive core.
			return result.err
		} else if exited {
			// case 4 - The hcnet core process exited unexpectedly without an error message
			return errors.Errorf("hcnet core exited unexpectedly")
		} else if !ok {
...
func (c *CaptiveHcnetCore) GetLatestLedgerSequence(ctx context.Context) (uint32, error) {
	...
	if c.prepared == nil {
		return 0, errors.New("hcnet-core must be prepared, call PrepareRange first")
	}
	if c.hcnetCoreRunner == nil {
		return 0, errors.New("hcnet-core cannot be nil, call PrepareRange first")
	}
	if c.closed {
		return 0, errors.New("hcnet-core is closed, call PrepareRange first")
...
// temporary files. Note, once a CaptiveHcnetCore instance is closed it can no longer be used and
...
func (c *CaptiveHcnetCore) Close() error {
	...
	// after the CaptiveHcnetCore context is canceled all subsequent calls to PrepareRange() will fail
	c.cancel()
	...
	if c.hcnetCoreRunner != nil {
		return c.hcnetCoreRunner.close()
	}
	return nil
}
