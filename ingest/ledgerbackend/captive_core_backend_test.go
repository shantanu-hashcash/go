...
func (m *hcnetCoreRunnerMock) context() context.Context {
	a := m.Called()
...
func (m *hcnetCoreRunnerMock) getMetaPipe() <-chan metaResult {
	a := m.Called()
	return a.Get(0).(<-chan metaResult)
}

func (m *hcnetCoreRunnerMock) getProcessExitError() (bool, error) {
	a := m.Called()
...
func TestCaptivePrepareRangeCrash(t *testing.T) {
	...
	err := captiveBackend.PrepareRange(ctx, BoundedRange(100, 200))
	assert.EqualError(t, err, "Error fast-forwarding to 100: hcnet core exited unexpectedly: exit code -1")
	mockRunner.AssertExpectations(t)
	mockArchive.AssertExpectations(t)
}
...
func TestCaptivePrepareRange_ErrCatchup(t *testing.T) {
	...
	assert.EqualError(t, err, "error starting prepare range: opening subprocess: error running hcnet-core: transient error")
...
func TestCaptiveHcnetCore_PrepareRangeAfterClose(t *testing.T) {
	...
	assert.NoError(t, captiveHcnetCore.Close())

	assert.EqualError(
		t,
		captiveHcnetCore.PrepareRange(ctx, BoundedRange(65, 66)),
		"error starting prepare range: opening subprocess: error getting latest checkpoint sequence: "+
			"error getting root HAS: Get \"http://localhost/.well-known/hcnet-history.json\": context canceled",
	)
	...
	captiveHcnetCore.archive = mockArchive
	assert.EqualError(
		t,
		captiveHcnetCore.PrepareRange(ctx, BoundedRange(65, 66)),
		"error starting prepare range: opening subprocess: error running hcnet-core: context canceled",
	)
	mockArchive.AssertExpectations(t)
}
	...
	return []GetLedgerTerminatedTestCase{
		{
			...
			true,
			nil,
			"hcnet core exited unexpectedly",
		},
		{
			"hcnet core exited unexpectedly with an error",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
		...
		{
			"hcnet core exited unexpectedly without error and closed channel",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
			nil,
		...
		{
			"hcnet core exited unexpectedly with an error and closed channel",
			context.Background(),
			[]metaResult{{LedgerCloseMeta: &ledger64}},
			true,
			fmt.Errorf("signal kill"),
