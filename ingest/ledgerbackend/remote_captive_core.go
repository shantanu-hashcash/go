...
// in the backend.
func (c RemoteCaptiveHcnetCore) GetLatestLedgerSequence(ctx context.Context) (sequence uint32, err error) {
...
func (c RemoteCaptiveHcnetCore) IsPrepared(ctx context.Context, ledgerRange Range) (bool, error) {
...
// GetLedger long-polls a remote hcnet core backend, until the requested
// ledger is ready.
...
func (c RemoteCaptiveHcnetCore) GetLedger(ctx context.Context, sequence uint32) (xdr.LedgerCloseMeta, error) {
	for {
		// TODO: Have some way to cancel all outstanding requests, not just
		// PrepareRange.
		u := *c.url
