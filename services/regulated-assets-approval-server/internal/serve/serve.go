...
import (
	...
	"github.com/sanjayhashcash/go/services/regulated-assets-approval-server/internal/db"
...
func handleHTTP(opts Options) http.Handler {
	...
	if err != nil {
		log.Fatal(errors.Wrapf(err, "%s cannot be parsed as a Hcnet amount", opts.KYCRequiredPaymentAmountThreshold))
	}
	...
	mux.Get("/.well-known/hcnet.toml", hcnetTOMLHandler{
	...
	mux.Route("/kyc-status", func(mux chi.Router) {
		...
		mux.Get("/{hcnet_address_or_callback_id}", kycstatus.GetDetailHandler{
			DB: db,
		}.ServeHTTP)
		mux.Delete("/{hcnet_address}", kycstatus.DeleteHandler{
			DB: db,
		}.ServeHTTP)
	})
