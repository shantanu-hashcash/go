...
type kycGetResponse struct {
	HcnetAddress string     `json:"hcnet_address"`
...
type getDetailRequest struct {
	HcnetAddressOrCallbackID string `path:"hcnet_address_or_callback_id"`
}
...
func (h GetDetailHandler) handle(ctx context.Context, in getDetailRequest) (*kycGetResponse, error) {
	// Check if getDetailRequest HcnetAddressOrCallbackID value is present.
	if in.HcnetAddressOrCallbackID == "" {
		return nil, httperror.NewHTTPError(http.StatusBadRequest, "Missing hcnet address or callbackID.")
	}

	// Prepare SELECT query return values.
	var (
		hcnetAddress, callbackID                        string
		emailAddress                                      sql.NullString
		createdAt                                         time.Time
		kycSubmittedAt, approvedAt, rejectedAt, pendingAt sql.NullTime
	)
	const q = `
		SELECT hcnet_address, email_address, created_at, kyc_submitted_at, approved_at, rejected_at, pending_at, callback_id
		FROM accounts_kyc_status
		WHERE hcnet_address = $1 OR callback_id = $1
	`
	err := h.DB.QueryRowContext(ctx, q, in.HcnetAddressOrCallbackID).Scan(&hcnetAddress, &emailAddress, &createdAt, &kycSubmittedAt, &approvedAt, &rejectedAt, &pendingAt, &callbackID)
