...
import (
	...
	"github.com/sanjayhashcash/go/services/regulated-assets-approval-server/internal/serve/kycstatus"
...
func TestAPI_postKYCStatus(t *testing.T) {
	...
	q = `
		...
		WHERE hcnet_address = $1 AND callback_id = $2
	`
...
func TestAPI_getKYCStatus(t *testing.T) {
	...
	m := chi.NewMux()
	m.Get("/kyc-status/{hcnet_address_or_callback_id}", handler.ServeHTTP)

	// step 1: insert data into database
	const q = `
		INSERT INTO accounts_kyc_status (hcnet_address, callback_id, email_address, created_at, kyc_submitted_at, rejected_at, pending_at, approved_at)
		VALUES
			('rejected-hcnet-address', 'rejected-callback-id', 'xrejected@test.com', $1::timestamptz, $2::timestamptz, $2::timestamptz, NULL, NULL),
			('pending-hcnet-address', 'pending-callback-id', 'ypending@test.com', $1::timestamptz, $3::timestamptz, NULL, $3::timestamptz, NULL),
			('approved-hcnet-address', 'approved-callback-id', 'approved@test.com', $1::timestamptz, $4::timestamptz, NULL, NULL, $4::timestamptz)
	`
	...
	r := httptest.NewRequest("GET", "/kyc-status/rejected-hcnet-address", nil)
	r = r.WithContext(ctx)
	w := httptest.NewRecorder()
	m.ServeHTTP(w, r)
	resp := w.Result()
	...
	wantBody := fmt.Sprintf(`{
		"hcnet_address": "rejected-hcnet-address",
	...
	r = httptest.NewRequest("GET", "/kyc-status/pending-hcnet-address", nil)
	r = r.WithContext(ctx)
	w = httptest.NewRecorder()
	m.ServeHTTP(w, r)
	resp = w.Result()
	...
	require.NoError(t, err)
	wantBody = fmt.Sprintf(`{
		"hcnet_address": "pending-hcnet-address",
	...
	r = httptest.NewRequest("GET", "/kyc-status/approved-hcnet-address", nil)
	r = r.WithContext(ctx)
	w = httptest.NewRecorder()
	m.ServeHTTP(w, r)
	resp = w.Result()
	...
	require.NoError(t, err)
	wantBody = fmt.Sprintf(`{
		"hcnet_address": "approved-hcnet-address",
...
func TestAPI_deleteKYCStatus(t *testing.T) {
	...
	m := chi.NewMux()
	m.Delete("/kyc-status/{hcnet_address}", handler.ServeHTTP)

	q := `
		INSERT INTO accounts_kyc_status (hcnet_address, callback_id, email_address, kyc_submitted_at, approved_at, rejected_at, pending_at)
