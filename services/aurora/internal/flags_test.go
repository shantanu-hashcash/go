...
func Test_createCaptiveCoreDefaultConfig(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			config: Config{Network: HcnetTestnet,
		...
		},
		{
			...
			config: Config{Network: HcnetPubnet,
				CaptiveCoreBinaryPath: "/path/to/captive-core/binary",
			},
	...
	tests := []struct {
		...
		},
		{
			...
			errStr: fmt.Sprintf("invalid captive core toml file: invalid captive core toml: "+
				"NETWORK_PASSPHRASE in captive core config file: %s does not match Aurora "+
...
func TestEnvironmentVariables(t *testing.T) {
	...
	assert.Equal(t, config.HcnetCoreURL, "http://localhost:11626")
...
func TestRemovedFlags(t *testing.T) {
	tests := []struct {
		...
		{
			name: "HCNET_CORE_DATABASE_URL removed",
			environmentVars: map[string]string{
				"INGEST":                    "false",
				"HCNET_CORE_DATABASE_URL": "coredb",
				"DATABASE_URL":              "dburl",
			},
			errStr: "flag --hcnet-core-db-url and environment variable HCNET_CORE_DATABASE_URL have been removed and no longer valid, must use captive core configuration for ingestion",
		},
		{
			name: "--hcnet-core-db-url  removed",
			environmentVars: map[string]string{
				"INGEST":       "false",
				"DATABASE_URL": "dburl",
			},
			errStr:  "flag --hcnet-core-db-url and environment variable HCNET_CORE_DATABASE_URL have been removed and no longer valid, must use captive core configuration for ingestion",
			cmdArgs: []string{"--hcnet-core-db-url=coredb"},
		},
		{
