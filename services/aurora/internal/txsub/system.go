...
func (sys *System) Submit(
	...
	if sr.Err != nil {
		...
		// Even if a transaction is successfully submitted to core, Aurora ingestion might
...
func (sys *System) waitUntilAccountSequence(ctx context.Context, db AuroraDB, sourceAddress string, seq uint64) error {
...
// isSyncedUp Check if Aurora and Core have synced up: If yes, then no need to wait for account sequence
