...
func startHTTPTest(t *testing.T, scenario string) *HTTPT {
	...
	ret.App = NewTestApp(tdb.AuroraURL())
	...
	ret.App.config.HcnetCoreURL = ret.coreServer.URL
	ret.App.UpdateCoreLedgerState(context.Background())
	ret.App.UpdateHcnetCoreInfo(context.Background())
...
func (ht *HTTPT) ReapHistory(retention uint) {
	...
	ht.App.reaper.HistoryQ = &history.Q{ht.AuroraSession()}
