...
func (s *system) verifyState(verifyAgainstLatestCheckpoint bool) error {
	...
	if verifyAgainstLatestCheckpoint {
		...
		for {
			...
			localLog.Info("Waiting for hcnet-core to publish HAS...")
			select {
			case <-ctx.Done():
				localLog.Info("State verifier shut down...")
				return nil
			case <-time.After(5 * time.Second):
				// Wait for hcnet-core to publish HAS
				retries++
				if retries == 12 {
	...
	verifier := verify.NewStateVerifier(stateReader, func(entry xdr.LedgerEntry) (bool, xdr.LedgerEntry) {
		...
		// Won't be persisting protocol 20 ContractData ledger entries (except for Hcnet Asset Contract
	...
	for {
		...
		for _, entry := range entries {
			...
			case xdr.LedgerEntryTypeTtl:
				...
				// we will only verify expiration of contract balances in the aurora db.
				if err = verifier.Write(entry); err != nil {
					return err
				}
				totalByType["ttl"]++
...
func checkAssetStats(
	...
	// only check contract asset balances which belong to hcnet asset contracts
