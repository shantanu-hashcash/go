...
var (
	// https://github.com/sanjayhashcash/rs-soroban-env/blob/v0.0.16/soroban-env-host/src/native_contract/token/public_types.rs#L22
	nativeAssetSym = xdr.ScSymbol("Native")
	// these are storage DataKey enum
	// https://github.com/sanjayhashcash/rs-soroban-env/blob/v0.0.16/soroban-env-host/src/native_contract/token/storage_types.rs#L23
...
// corresponds to the asset info entry written to contract storage by the Hcnet
// Asset Contract upon initialization.
//
// Note that AssetFromContractData will ignore forged asset info entries by
// deriving the Hcnet Asset Contract ID from the asset info entry and comparing
// it to the contract ID found in the ledger entry.
//
// If the given ledger entry is a verified asset info entry,
// AssetFromContractData will return the corresponding Hcnet asset. Otherwise,
// it returns nil.
//
// References:
// https://github.com/sanjayhashcash/rs-soroban-env/blob/v0.0.16/soroban-env-host/src/native_contract/token/public_types.rs#L21
// https://github.com/sanjayhashcash/rs-soroban-env/blob/v0.0.16/soroban-env-host/src/native_contract/token/asset_info.rs#L6
// https://github.com/sanjayhashcash/rs-soroban-env/blob/v0.0.16/soroban-env-host/src/native_contract/token/contract.rs#L115
//
...
// the Hcnet Asset Contract.
//
// Reference:
//
//	https://github.com/sanjayhashcash/rs-soroban-env/blob/da325551829d31dcbfa71427d51c18e71a121c5f/soroban-env-host/src/native_contract/token/storage_types.rs#L11-L24
func ContractBalanceFromContractData(ledgerEntry xdr.LedgerEntry, passphrase string) ([32]byte, *big.Int, bool) {
	...
	// https://github.com/sanjayhashcash/rs-soroban-env/blob/a66f0815ba06a2f5328ac420950690fd1642f887/soroban-env-host/src/native_contract/token/balance.rs#L92-L93
			...
			Val: xdr.ScVal{
				...
				Instance: &xdr.ScContractInstance{
					Executable: xdr.ContractExecutable{
						Type: xdr.ContractExecutableTypeContractExecutableHcnetAsset,
					},
					Storage: storageMap,
				},
			},
...
// written to contract storage by the Hcnet Asset Contract.
//
// Warning: Only for use in tests. This does not create the accompanied TTLEntry which would typically be created by core.
