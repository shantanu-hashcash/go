...
func Flags() (*Config, support.ConfigOptions) {
	...
	var flags = support.ConfigOptions{
		...
		&support.ConfigOption{
			Name:           HcnetCoreBinaryPathName,
			OptType:        types.String,
			FlagDefault:    "",
			Required:       false,
			Usage:          "path to hcnet core binary, look for the hcnet-core binary in $PATH by default.",
		...
		&support.ConfigOption{
			...
			FlagDefault:    false,
			Required:       false,
			Usage:          "disables the transaction submission functionality of Aurora.",
		...
		&support.ConfigOption{
			...
			FlagDefault: true,
			Required:    false,
			Usage: `when enabled, Aurora ingestion will instruct the captive
				...
				if val := viper.GetString(opt.Name); val != "" {
					stdLog.Printf(
						"DEPRECATED - The usage of the flag --enable-captive-core-ingestion has been deprecated. " +
							"Aurora now uses Captive-Core ingestion by default and this flag will soon be removed in " +
							"the future.",
					)
				}
				return nil
		...
		&support.ConfigOption{
			...
			CustomSetValue: support.SetOptionalUint,
			Required:       false,
			Usage:          "port for Captive Core to bind to for connecting to the Hcnet swarm (0 uses Hcnet Core's default)",
			ConfigKey:      &config.CaptiveCoreTomlParams.PeerPort,
			UsedInCommands: IngestionCommands,
		},
		&support.ConfigOption{
			Name:     HcnetCoreDBURLFlagName,
			...
			CustomSetValue: func(opt *support.ConfigOption) error {
				if val := viper.GetString(opt.Name); val != "" {
					return fmt.Errorf("flag --hcnet-core-db-url and environment variable HCNET_CORE_DATABASE_URL have been removed and no longer valid, must use captive core configuration for ingestion")
				}
				return nil
			},
			UsedInCommands: IngestionCommands,
		...
		&support.ConfigOption{
			Name:           HcnetCoreURLFlagName,
			ConfigKey:      &config.HcnetCoreURL,
			OptType:        types.String,
			Usage:          "hcnet-core to connect with (for http commands). If unset and the local Captive core is enabled, it will use http://localhost:<hcnet_captive_core_http_port>",
			UsedInCommands: IngestionCommands,
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			FlagDefault: uint(0),
			Required:    false,
			Usage: "The maximum number of path finding requests per second aurora will allow." +
		...
		&support.ConfigOption{
			...
			OptType:        types.Bool,
			FlagDefault:    false,
			Usage:          "disable periodic verification of ledger state in aurora db (not recommended)",
			UsedInCommands: IngestionCommands,
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			...
			CustomSetValue: func(co *support.ConfigOption) error {
				...
				if val != "" && val != HcnetPubnet && val != HcnetTestnet {
					return fmt.Errorf("invalid network %s. Use '%s' or '%s'",
						val, HcnetPubnet, HcnetTestnet)
				}
				*co.ConfigKey.(*string) = val
				return nil
			},
			Usage: fmt.Sprintf("hcnet public network, either '%s' or '%s'."+
				" It automatically configures network settings, including %s, %s, and %s.",
				HcnetPubnet, HcnetTestnet, NetworkPassphraseFlagName,
...
func NewAppFromFlags(config *Config, flags support.ConfigOptions) (*App, error) {
	...
	if (!config.DisableTxSub || config.Ingest) && config.HcnetCoreURL == "" {
		return nil, fmt.Errorf("flag --%s cannot be empty", HcnetCoreURLFlagName)
	}

	log.Infof("Initializing aurora...")
...
func getCaptiveCoreBinaryPath() (string, error) {
	result, err := exec.LookPath("hcnet-core")
	if err != nil {
		return "", err
	}
	return result, nil
...
func setCaptiveCoreConfiguration(config *Config, options ApplyOptions) error {
	...
	// If the user didn't specify a Hcnet Core binary, we can check the
	...
	if config.CaptiveCoreBinaryPath == "" {
		...
		if config.CaptiveCoreBinaryPath, err = getCaptiveCoreBinaryPath(); err != nil {
			return fmt.Errorf("captive core requires %s", HcnetCoreBinaryPathName)
		}
	}
	...
	// point to it.
	if config.HcnetCoreURL == "" && config.CaptiveCoreToml.HTTPPort != 0 {
		config.HcnetCoreURL = fmt.Sprintf("http://localhost:%d", config.CaptiveCoreToml.HTTPPort)
	}
...
func ApplyFlags(config *Config, flags support.ConfigOptions, options ApplyOptions) error {
	...
	if len(flagsPassedByUser) > 0 {
		result := fmt.Sprintf("DEPRECATED - the use of command-line flags: [%s], has been deprecated in favor of environment variables. "+
			"Please consult our Configuring section in the developer documentation on how to use them - https://developers.hcnet.org/docs/run-api-server/configuring", "--"+strings.Join(flagsPassedByUser, ",--"))
	...
	if config.BehindCloudflare && config.BehindAWSLoadBalancer {
		return fmt.Errorf("invalid config: Only one option of --behind-cloudflare and --behind-aws-load-balancer is allowed." +
			" If Aurora is behind both, use --behind-cloudflare only")
	}
