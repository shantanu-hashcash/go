...
// Refer to ./services/aurora/internal/integration/contracts/README.md on how to recompile
...
func CaseContractMintToAccount(t *testing.T) {
	...
	assertAssetStats(itest, assetStats{
		...
		contractID:               hcnetAssetContractID(itest, asset),
	})
...
func CaseExpirationAndRestoration(t *testing.T) {
	...
	itest := integration.NewTest(t, integration.Config{
		...
		AuroraIngestParameters: map[string]string{
			...
			// a fake asset contract in the aurora db and we don't
	...
	_, err := itest.AuroraIngest().HistoryQ().InsertAssetStat(
		context.Background(),
		syntheticAssetStat,
	)
	assert.NoError(t, err)
	...
	// The TESTING_MINIMUM_PERSISTENT_ENTRY_LIFETIME=10 configuration in hcnet-core
...
func assertAssetStats(itest *integration.Test, expected assetStats) {
	assets, err := itest.Client().Assets(auroraclient.AssetRequest{
...
func getTxEffects(itest *integration.Test, txHash string, asset xdr.Asset) []effects.Effect {
	t := itest.CurrentTest()
	effects, err := itest.Client().Effects(auroraclient.EffectRequest{
...
func mintWithAmt(itest *integration.Test, sourceAccount string, asset xdr.Asset, assetAmount xdr.ScVal, recipient xdr.ScVal) *txnbuild.InvokeHostFunction {
	invokeHostFn := &txnbuild.InvokeHostFunction{
		HostFunction: xdr.HostFunction{
			Type: xdr.HostFunctionTypeHostFunctionTypeInvokeContract,
			InvokeContract: &xdr.InvokeContractArgs{
				ContractAddress: contractIDParam(hcnetAssetContractID(itest, asset)),
				FunctionName:    "mint",
				Args: xdr.ScVec{
					recipient,
					assetAmount,
...
func initAssetContract(itest *integration.Test, sourceAccount string, asset xdr.Asset, sacTestcontractID, sacTestcontractHash xdr.Hash) *txnbuild.InvokeHostFunction {
	targetContract := contractIDParam(hcnetAssetContractID(itest, asset))
	invokeHostFn := &txnbuild.InvokeHostFunction{
		HostFunction: xdr.HostFunction{
			Type: xdr.HostFunctionTypeHostFunctionTypeInvokeContract,
			InvokeContract: &xdr.InvokeContractArgs{
...
func hcnetAssetContractID(itest *integration.Test, asset xdr.Asset) xdr.Hash {
