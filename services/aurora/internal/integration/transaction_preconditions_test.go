...
func TestTransactionPreconditionsLedgerBounds(t *testing.T) {
	...
	//verify roundtrip to network and back through the aurora api returns same precondition values
	txHistory, err := itest.Client().TransactionDetail(tx.Hash)
	assert.NoError(t, err)
...
// TestTransactionWithoutPreconditions ensures that Aurora doesn't break when
...
func TestTransactionPreconditionsEdgeCases(t *testing.T) {
	...
	for _, precondition := range preconditionTests {
		...
		// but rather to just make sure the edge case doesn't crash Aurora.
