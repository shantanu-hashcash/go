...
func validateHcnetPublicKey(publicKey string) error {
	if publicKey == "" {
		return errors.New("public key is undefined")
	}

	if !strkey.IsValidEd25519PublicKey(publicKey) {
		return errors.Errorf("%s is not a valid hcnet public key", publicKey)
	}
	return nil
}

// validateHcnetSignerKey returns an error if a signerkey is invalid. Otherwise, it returns nil.
func validateHcnetSignerKey(signerKey string) error {
	if signerKey == "" {
	...
	if err := xdrKey.SetAddress(signerKey); err != nil {
		return errors.Errorf("%s is not a valid hcnet signer key", signerKey)
	}
	return nil
}

// validateHcnetAsset checks if the asset supplied is a valid hcnet Asset. It returns an error if the asset is
// nil, has an invalid asset code or issuer.
func validateHcnetAsset(asset BasicAsset) error {
	if asset == nil {
	...
	err = validateHcnetPublicKey(asset.GetIssuer())
	if err != nil {
...
// validateAmount checks if the provided value is a valid hcnet amount, it returns an error if not.
// This is used to validate price and amount fields in structs.
func validateAmount(n interface{}) error {
	var hcnetAmount int64
...
func validateAssetCode(asset BasicAsset) error {
	// Note: we are not using validateHcnetAsset() function for AllowTrust operations because it requires the
	//  following :
	// - asset is non-native
	// - asset code is valid
...
func validateChangeTrustAsset(asset ChangeTrustAsset) error {
	// Note: we are not using validateHcnetAsset() function for ChangeTrust operations because it requires the
	//  following :
	// - asset is non-native
	// - asset code is valid
...
// It checks that the buying and selling assets are valid hcnet assets, and that amount and price are valid.
...
func validatePassiveOffer(buying, selling Asset, offerAmount string, price xdr.Price) error {
	...
	// https://github.com/sanjayhashcash/go/pull/1707#discussion_r321508440
	err := validateHcnetAsset(buying)
	if err != nil {
		return NewValidationError("Buying", err.Error())
	}

	err = validateHcnetAsset(selling)
	if err != nil {
...
// It checks that the buying and selling assets are valid hcnet assets, and that amount, price and offerID
...
// https://github.com/sanjayhashcash/hcnet-protocol/blob/master/ecosystem/sep-0011.md#asset
