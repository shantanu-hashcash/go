...
func TestValidateHcnetPublicKey(t *testing.T) {
	validKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7YFMTLB65PYM"
	err := validateHcnetPublicKey(validKey)
	assert.NoError(t, err, "public key should be valid")

	invalidKey := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y"
	err = validateHcnetPublicKey(invalidKey)
	expectedErrMsg := "GDWZCOEQRODFCH6ISYQPWY67L3ULLWS5ISXYYL5GH43W7Y is not a valid hcnet public key"
	...
	err = validateHcnetPublicKey(invalidKey)
	expectedErrMsg = "SBCVMMCBEDB64TVJZFYJOJAERZC4YVVUOE6SYR2Y76CBTENGUSGWRRVO is not a valid hcnet public key"
	require.EqualError(t, err, expectedErrMsg, "public key should be invalid")
}

func TestValidateHcnetAssetWithValidAsset(t *testing.T) {
	nativeAsset := NativeAsset{}
	err := validateHcnetAsset(nativeAsset)
	assert.NoError(t, err)

	kp0 := newKeypair0()
	creditAsset := CreditAsset{"XYZ", kp0.Address()}
	err = validateHcnetAsset(creditAsset)
	assert.NoError(t, err)
}

func TestValidateHcnetAssetWithInValidAsset(t *testing.T) {
	err := validateHcnetAsset(nil)
	assert.Error(t, err)
	...
	err = validateHcnetAsset(creditAssetNoCode)
	assert.Error(t, err)
	...
	err = validateHcnetAsset(creditAssetNoIssuer)
	assert.Error(t, err)
...
func TestValidateAmountInvalidValue(t *testing.T) {
	...
	require.EqualError(t, err, expectedErrMsg, "should be a valid hcnet amount")
